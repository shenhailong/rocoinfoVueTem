<template>
  <div>
    <div class="swiper-container swiper-container-focus " ref="peopleSwiper">
      <div class="swiper-wrapper">
        <div class="swiper-slide" v-for="(items) in peopleList" :key="items.id" @click="personal(items.targetMember.id)">
          <div class="item-goods item-goods-author">
            <div class="author-column">
              <div class="author-avatar">
                <img v-lazy="{src: items.targetMember.profileImage ? items.targetMember.profileImage : 'static/images/m/avatar.jpg',
                loading: 'static/images/m/avatar.jpg',
                error: 'static/images/m/avatar.jpg'}" alt="商品图片" class="lazy-img-fadein">
                <svg class="svg-member-circle" v-if="items.targetMember.type === 'OFFICIAL'">
                  <use xlink:href="#svg-member-circle" />
                </svg>
                <div class="member-grade" v-if="items.targetMember.type !== 'OFFICIAL' && items.targetMember.level">
                  <svg class="svg-member-grade">
                    <use xlink:href="#svg-member-grade"></use>
                  </svg>
                  <span class="num">{{items.targetMember.level.levelNum - 1 }}</span>
                </div>
              </div>
            </div>
            <div class="item-title _ellipsis">{{items.targetMember.nickname}}</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getPeople() {
      let self = this
      let promise = axios.get('/api/wap/forum/myFollows?limit=12').then((res) => {
        if (res.data.code === CODE_SUCCESS) {
          self.peopleList = res.data.data.page.rows
          this.$nextTick(() => {
            this.peoplesSwiper = new Swiper(this.$refs.peopleSwiper, {
              slidesPerView: 'auto'
            })
          })
        }
