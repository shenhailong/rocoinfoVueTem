  ...
  <div class="wrap wrap-edit-topic">
    ...
    <div class="content">
      <form action="" class="form-normal form-eidt-topic">
        ...
        <image-picker :files="files" :max="5" :canCrop="canCrop" @exceed-max="onExceedMax" @add-img="onAddImage" @remove-img="onRemoveImage" :fixedNumber="[750, 562]" :autoCropWidth="750" :autoCropHeight="562" :fixedBox="true" @startLoading="startLoading" @endLoading="endLoading"/>
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      let promise = axios.get(`/api/wap/topic/${this.$route.query.id}/get`).then((res) => {
        if (res.data.code === CODE_SUCCESS) {
          this.sendData = res.data.data.topic
          this.sendData.sku = {}
    ...
    saveDraft() {
      ...
      promise = axios.post('/api/wap/topic/modify', this.sendData).then((res) => {
        if (res.data.code === CODE_SUCCESS) {
